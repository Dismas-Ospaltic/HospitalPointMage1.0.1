...
if($hospital_patient_no !=""){
    $select_single_inco_patient_data = mysqli_query($conn, "SELECT * FROM patient_laboratory WHERE request_date='{$current_date}' AND status='{$tested}' AND hospital_patient_no='{$hospital_patient_no}'");
    ...
    if(mysqli_num_rows($select_single_inco_patient_data) > 0){
     ...
     $select_specified_test = mysqli_query($conn, "SELECT sample_collected FROM patient_laboratory WHERE hospital_patient_no='{$row_fetch_lab['hospital_patient_no']}' AND request_date='{$current_date}'");
   
     $data1=mysqli_fetch_array( $select_specified_test);

     $res1=$data1['sample_collected'];
     ...
     for($i=0; $i<=$count1;$i++){
        ...
        $select_specified_test = mysqli_query($conn, "SELECT test_list FROM patient_laboratory WHERE hospital_patient_no='{$row_fetch_lab['hospital_patient_no']}' AND request_date='{$current_date}'");
      
        $data2=mysqli_fetch_array($select_specified_test);

        $res2=$data2['test_list'];
        ...
        $select_specified_test = mysqli_query($conn, "SELECT test_results FROM patient_laboratory WHERE hospital_patient_no='{$row_fetch_lab['hospital_patient_no']}' AND request_date='{$current_date}'");
        
        $data3=mysqli_fetch_array($select_specified_test);

        $res3=$data3['test_results'];
        ...
        for($i=0; $i<=$count3;$i++){
           ...
           $select_specified_test = mysqli_query($conn, "SELECT other_conclusion FROM patient_laboratory WHERE hospital_patient_no='{$row_fetch_lab['hospital_patient_no']}' AND request_date='{$current_date}'");
        
           $data3=mysqli_fetch_array($select_specified_test);
   
           $res3=$data3['other_conclusion'];
